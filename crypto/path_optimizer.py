...
class PathOptimizer(Model):
	...
	def update_objectives(self):
		...
		#print(dumps(self.transit_price_matrix))
		print(sys.path);
		#print(dumps(self.commission_matrix));
		#print(dumps(self.vol_matrix));
		#print(dumps(self.min_trading_limit));
		sys.exit();
	...
	def update_vol_matrix(self, percentile=0.01):
		...
		for key, val in self.price.items():
			...
			if base_coin in self.crypto_prices:
				#print('self.crypto_price '+base_coin+':'+str(self.crypto_prices[base_coin]['price'])+', percentile'+str(percentile))
				if self.crypto_prices[base_coin]['price']!=None:
