...
def get_crypto_prices(coin_set, convert='USD'):
	...
	#else:
    #
	coin_set = set([i for i in coin_set if i.isalnum() and i.isupper() and ',AERO,ATMCASH,AXIS,BQX,CAI,CNOTE,DNS,DVK,FAC,FVZ,GIAR,GPU,GXS,HSR,IOTA,ITM,JUG,KFEE,KRW,MZC,MYR,NBT,NL,PHB,PROM,RPX,RUB,SQL,SRCC,TWE,UAH,USDSB,VOX,WOLF,X13,XHC,XVN,XXC,'.find(','+i+',')==-1])
	output = {}
	...
	headers = {
		...
		'X-CMC_PRO_API_KEY': 'b801c7a4-47ef-4626-b3e1-41e00fda69cb',
	}
	#print(parameters)
	session = Session()
...
def save_to_file(fileName,content):
	...
	with open(fileName, 'w') as f:
		to_save = original + content
		f.write(to_save)


def save_record(fileName, path_optimizer, amt_optimizer):
